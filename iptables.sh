...
IPSET_LANIPLIST="laniplist"  # 网页端填写的地址
IPSET_VPSIPLIST="vpsiplist"  # VPS地址，url转IP
IPSET_SHUNTLIST="shuntlist"  # 0
IPSET_GFW="gfwlist" # 比较多 Number of entries: 2729 
IPSET_CHN="chnroute"  # 比较多 Number of entries: 8635
IPSET_BLACKLIST="blacklist" # 205个 应该是谷歌什么的
IPSET_WHITELIST="whitelist" # 应该是规则列表里的一些
IPSET_BLOCKLIST="blocklist"
# 感觉以上是把那些网页端列表里的都给写了个遍。URL就写URL的IP，但是不知道为啥gfwlist里会这么多，可能把geoip都给放进来了
...
add_firewall_rule() {
	...
	ipset -! create $IPSET_LANIPLIST nethash maxelem 1048576
		# ipset -! 忽略错误
		# create $IPSET_LANIPLIST IPSET_LANIPLIST="laniplist"
		# create SETNAME    TYPENAME [type-specific-options]
		# 	$IPSET_LANIPLIST nethash maxelem 1048576
